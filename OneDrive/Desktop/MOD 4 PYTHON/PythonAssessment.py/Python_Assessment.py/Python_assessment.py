...
def identify_most_common_word(text):
    ...
    for word in word_counts:
        if word_counts[word] > highest:
            highest = word_counts[word]
            most_common = word
...
def calculate_average_word_length(text):
    ...
    while i < len(words):
        clean = words[i].strip("\'")
        if clean != "":
            total_length = total_length + len(clean)
...
# check if file has content using if/else
if article.strip() == "":
    ...
    # call all the functions and print results
    print("Word count for '" + search_word + "':", count_specific_word(article, search_word))
